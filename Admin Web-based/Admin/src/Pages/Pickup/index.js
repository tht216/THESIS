...
import {
  asyncCreatePickupAction,
  asyncDeletePickupAction,
  asyncEditPickupAction,
  asyncGetAllPickupAction,
  asyncGetDetailPickupAction,
} from "./store/action";
...
const Pickup = (props) => {
  ...
  const onEdit = async (id) => {
    setIsEdit(true);
    setId(id);
    const response = await getDetailPickup(id);
    if (response?.status === 200) {
      // console.log({
  ...
  const onFinish = async (values) => {
    console.log(values);
    if (isEdit) {
      const response = await editPickup({ form: values, id: id });
      if (response.status === 200) {
        await getAllPickup({
          search: payload.search,
          sort: payload.sorting.field,
      ...
      const response = await createPickup(values);
      console.log(response);
  ...
  return (
    <div>
      ...
      <Modal
        title={isEdit ? "Edit Pickup" : "Create Pickup"}
        open={visible}
          ...
          >
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Pickup Type is required",
                },
              ]}
...
const mapStateToProps = createStructuredSelector({
  ...
  PickupList: selectAllPickup,
  total: selectTotal,
  detail: selectDetail,
});

const mapDispatchToProps = (dispatch) => ({
  getAllPickup: (payload) => asyncGetAllPickupAction(dispatch)(payload),
  getDetailPickup: (payload) => asyncGetDetailPickupAction(dispatch)(payload),
  createPickup: (payload) => asyncCreatePickupAction(dispatch)(payload),
  deletePickup: (payload) => asyncDeletePickupAction(dispatch)(payload),
  editPickup: (payload) => asyncEditPickupAction(dispatch)(payload),
});

const withConnect = connect(mapStateToProps, mapDispatchToProps);

export default compose(withConnect)(Pickup);
